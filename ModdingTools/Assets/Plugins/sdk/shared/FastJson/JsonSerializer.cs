...
namespace fastJSON
{
    internal sealed class JSONSerializer
    {
        ...
        private void WriteValue(object obj)
        {
            ...
            else if (obj is float)
            {
                ...
                else if (float.IsInfinity(d))
                {
                    ...
                    _output.Append(((float)obj).ToString("0.0####",NumberFormatInfo.InvariantInfo));
            }
        ...
        {
            if(obj is ISerializationCallbackReceiver dd)
                dd.OnBeforeSerialize();
            int i = 0;
            ...
            if (_current_depth <= _MAX_DEPTH)
            {
                ...
                for (int ii = 0; ii < c; ii++)
                {
                    ...
                    if (_params.SerializeNullValues == false && (o == null || o is DBNull ||  _params.SkipDefaultValues && IsDefaultValue(obj,p,o)))
                        continue;
        ...
        public static Dictionary<Type, object> defaultValues2 = new Dictionary<Type, object>();
        public static Dictionary<(Type,String), object> defaultValues = new Dictionary<(Type,String),object>();
        public static bool IsDefaultValue(object _this,Getters getter, object o)
        {
            if (o is string s && s == "")
                return true;

            if (o is IList d && d.Count == 0)
                return true;
            var type = _this.GetType();

            if (!defaultValues.TryGetValue((type,getter.Name), out object b))
                b = defaultValues[(type,getter.Name)] = o.GetType().IsValueType ? getter.Getter(CreateInstance(type)) : null;
            
            return o.Equals(b);
        }
        private static object CreateInstance(Type type)
        {
            if (!defaultValues2.TryGetValue(type, out object inst))
                inst = defaultValues2[type] =Activator.CreateInstance(type);
            return inst;
        }
