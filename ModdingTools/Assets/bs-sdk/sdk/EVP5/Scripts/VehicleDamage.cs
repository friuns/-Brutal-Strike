...
public class VehicleDamage : MonoBehaviour
{
   ...
   internal float meshDamage = 0.0f;
   internal float colliderDamage = 0.0f;
   ...
   void OnEnable()
    ...
    void ProcessImpact()
    {
        ...
        if (m_vehicle.sumImpactVelocity.sqrMagnitude > minVelocity * minVelocity)
            impactVelocity = m_vehicle.cachedTransform.TransformDirection(m_vehicle.sumImpactVelocity) * multiplier * 0.02f;
        
        if (impactVelocity.sqrMagnitude > 0.0f)
        {
            Vector3 contactPoint = transform.TransformPoint(m_vehicle.sumImpactPosition);

            // Deform the meshes

            for (int i = 0, c = meshes.Length; i < c; i++)
                meshDamage += DeformMesh(meshes[i].mesh, m_originalMeshes[i], meshes[i].transform, contactPoint, impactVelocity);
...
// #if game
//             if (car)
//                 car.AddLife((colliderDamage)*10, m_vehicle.car);
// #endif    
        }
    }
